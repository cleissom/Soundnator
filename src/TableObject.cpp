...
TableObject::TableObject(int id, connectionType_t connection)
    : id(id),
      dobj(nullptr),
...
int TableObject::getId() { return this->id; }
...
void TableObject::updateTurnMultiplier(float angle) {
    ...
    if ((derivative > derivativeThreshold) && (((turnsMultiplier * M_2PI) - angle) < angleMaxValue))
        turnsMultiplier++;
    else if ((derivative < (-derivativeThreshold)) && (((2 * turnsMultiplier * M_2PI) - angle) > angleMinValue))
        turnsMultiplier--;
...
TableObject **TableObject::getPrecedingObj(TableObject *obj) { return getPrecedingObj(obj, this->connection); }
...
bool TableObject::isConnectedTo(TableObject *obj) { return obj == followingObj; }
...
bool TableObject::haveConnection() { return (this->followingObj != nullptr); }
...
void TableObject::setDirectObject(DirectObject *dobj) { this->dobj = dobj; }

DirectObject *TableObject::getDirectObject() { return dobj; }

TableObject *TableObject::getFollowingObject() { return followingObj; }
...
void TableObject::setToScope(pdsp::Patchable &in) { in >> scope >> SoundEngine::I().getEngine().blackhole(); }
...
bool Generator::objectIsConnectableTo(TableObject *obj) { return isObject<Effect>(obj); }

bool Generator::objectIsConnectableToOutput() { return true; }
...
void Oscillator::updateVolume(TableSlider::updateSliderArgs &a) { amp.set(a.value / 100.0f); }
...
void Sampler::updateVolume(TableSlider::updateSliderArgs &a) { amp.set(a.value / 100.0f); }

int Sampler::sumInstrumentSize(InstrumentType v) { return samples[v].size(); }
...
bool Effect::objectIsConnectableTo(TableObject *obj) { return isObject<Effect>(obj); }

bool Effect::objectIsConnectableToOutput() { return true; }
...
bool Delay::objectIsConnectableTo(TableObject *obj) { return false; }
...
bool Controller::objectIsConnectableToOutput() { return false; }

////////////////////////////////////////////////////////////////////////////////////////////////////////////

Sequencer::Sequencer(int id, int sequencerSection, connectionType_t connection)
    : Controller(id, connection), sequencerSection(sequencerSection) {
    patch();
...
void Sequencer::update() {
    ...
    tableSequencerCells->setActiveCell(
        int(ofMap(SoundEngine::I().getSection(sequencerSection).sequence(0).meter_percent(), 0, 0.95, 0, 15)));
}
...
void Sequencer::longPushButton(TableButton::LongPushButtonArgs &a) { showSlider = not(showSlider); }
...
Output::Output(int id) : TableObject(id) { patch(); }
...
bool Output::haveConnection() { return true; }

bool Output::objectIsConnectableTo(TableObject *obj) { return false; }

bool Output::objectIsConnectableToOutput() { return false; }
